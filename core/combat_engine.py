...
def make_player_combatant(character, row=FRONT):
    ...
    return {
        ...
        "momentum": 0,       # generated by basic attacks, spent on physical abilities
        "max_momentum": 6,   # pip cap
    }
...
def resolve_basic_attack(attacker, defender, enemies=None):
    ...
    if attacker.get("type") == "player":
        gain = 2 if is_crit else 1
        old_mom = attacker.get("momentum", 0)
        attacker["momentum"] = min(attacker.get("max_momentum", 6), old_mom + gain)
        result["momentum_gained"] = gain
        result["momentum_total"]  = attacker["momentum"]
    ...
    if resource_key and attacker["type"] == "player":
        if resource_key == "momentum":
            ...
            if current < cost:
                ...
                result["messages"].append(
                    f"{attacker['name']} needs {cost} Momentum but only has {current}! "
                    f"(Attack to build Momentum first)"
                )
                return result
            attacker["momentum"] = current - cost
            result["_resource_spent"] = ("Momentum", cost, attacker["momentum"])
        else:
    ...
    def advance_turn(self):
        ...
        if not any(e["alive"] for e in self.enemies):
            ...
            for p in self.players:
                p["momentum"] = 0  # momentum resets between fights
            self._calc_rewards()
            return
