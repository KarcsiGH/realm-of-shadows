...
DISEASE_EFFECTS = {
    # Disease differs from poison: slower ticks, drains max HP over time,
    # and persists out of combat (must be cured at temple/apothecary or with Panacea).
    # CON above 10 gives +2% resist per point when disease is inflicted.
    "disease_blight": {
        "name": "Blight",       "tier": 1,
        "dmg_per_tick": 3,      "tick_every_steps": 5,  "total_ticks": 6,
        "max_hp_drain": 2,      # also drains max HP each tick
        "save_stat": "CON",     "save_threshold": 55,
    },
    "disease_plague": {
        "name": "Plague",       "tier": 2,
        "dmg_per_tick": 7,      "tick_every_steps": 4,  "total_ticks": 8,
        "max_hp_drain": 4,
        "save_stat": "CON",     "save_threshold": 40,
    },
    "disease_wither": {
        "name": "Withering",    "tier": 3,
        "dmg_per_tick": 12,     "tick_every_steps": 3,  "total_ticks": 10,
        "max_hp_drain": 6,
        "save_stat": "CON",     "save_threshold": 25,
    },
}
...
def add_disease(character, disease_id):
    """Apply a disease effect. Returns True if applied.
    Disease persists out of combat — only temple/Panacea removes it.
    Higher-tier diseases replace lower ones."""
    if disease_id not in DISEASE_EFFECTS:
        return False
    ddef = DISEASE_EFFECTS[disease_id]
    effects = get_status_effects(character)
    for i, s in enumerate(effects):
        if s.get("type") == "disease":
            if ddef["tier"] > s.get("tier", 0):
                effects[i] = {
                    "id": disease_id, "type": "disease",
                    "name": ddef["name"], "tier": ddef["tier"],
                    "dmg": ddef["dmg_per_tick"],
                    "max_hp_drain": ddef.get("max_hp_drain", 0),
                    "tick_every": ddef["tick_every_steps"],
                    "ticks_left": ddef["total_ticks"],
                    "steps_since_tick": 0,
                }
                return True
            return False  # already have equal or stronger
...
def remove_all_disease(character):
    """Remove all disease effects (temple cure / Panacea)."""
    effects = get_status_effects(character)
    ...
    for s in effects:
        if s["type"] == "poison":
            ...
            if s["steps_since_tick"] >= s["tick_every"]:
                ...
                dmg = s["dmg"]
                character.resources["HP"] = character.resources.get("HP", 0) - dmg
                # Disease also drains max HP — the body weakens
                drain = s.get("max_hp_drain", 0)
                if drain and hasattr(character, "max_hp"):
                    character.max_hp = max(1, character.max_hp - drain)
                messages.append(f"{character.name} suffers {s['name']} — {dmg} damage, -{drain} max HP!")

                if character.resources["HP"] <= 0:
                    messages.append(f"{character.name} succumbs to {s['name']}!")

                if s["ticks_left"] <= 0:
                    to_remove.append(s["id"])
                    messages.append(f"{character.name}'s {s['name']} runs its course, but the damage is done.")
...
def get_status_display(character):
    ...
    for s in get_status_effects(character):
        if s["type"] == "poison":
            ...
            display.append((s["name"], (180, 100, 30)))  # sickly orange-brown
        elif s["type"] == "curse":
