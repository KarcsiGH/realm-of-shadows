...
try:
    ...
    check("40+ total NPCs across towns", total_npcs >= 40)
...
try:
    ...
    check("18+ quests defined", len(QUESTS) >= 18)
...
try:
    ...
    m.xp = 400; apply_level_up(m, "INT")   # xp_for_level(2) = 400
    m.xp = 900                              # xp_for_level(3) = 900
...
try:
    ...
    c1.xp = 400; apply_level_up(c1, "STR")   # xp_for_level(2) = 400
    c1.xp = 900; apply_level_up(c1, "STR")   # xp_for_level(3) = 900
    c1.gold = 750
    c1.inventory = [{"name": "Health Potion", "type": "consumable", "identified": True}]

    c2 = Character("Lyra", "Mage")
    c2.xp = 400; apply_level_up(c2, "INT")   # xp_for_level(2) = 400
...
try:
    ...
    # ── AoE ability routing ───────────────────────────────────────
    # AoE dispatch goes through _resolve_popover_item (popover architecture).
    ...
    if actor7:
        ...
        # AoE abilities auto-broadcast through _resolve_popover_item
        popover_data = {"type": "ability", "ability": aoe_ability}
        result7 = cui7._resolve_popover_item(popover_data, actor7)
